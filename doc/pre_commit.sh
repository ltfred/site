#!/usr/bin/env bash
# this pre-commit hook will run several checks
# go for a coffee if it's committing
#
# Install (*nix):
#   ln -sf $(pwd)/doc/pre-commit.sh .git/hooks/pre-commit
# Install (Windows):
#   mklink .git\hooks\pre-commit C:\Users\user\.pre-commit.sh
echo "Start pre-commit tasks..."
set -e
...
if [[ ! "${PYTHON_FILES}" ]];
then
  echo "No python file, finish pre-commit tasks!"
  exit 0
fi
